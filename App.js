...
class App extends React.Component<Props, State> {
  ...
  async componentDidMount() {
    ...
    this.animate(2, () => this.setState({ loaded: true }));
  }

  componentDidUpdate(prevProps: Props, prevState: State) {
    if (!prevState.loaded && this.state.loaded) this.fadeInNavigator();
  }
    ...
    return (
      <Provider store={store}>
        <ThemeProvider theme={theme}>
          {this.state.loaded ? (
            <Animated.View style={{ flex: 1, opacity: navigatorAnimation }}>
