...
class MorphingScreen extends React.Component<Props, State> {
  ...
  get transform(): Object[] {
    ...
    const {morphLayout: {x, y, w, h, px, py}} = this.props
  ...
  render() {
    ...
    const collapsedChildren = React.Children
      .toArray(children)
      .reduce(
        (prev, child) => child.type === MorphingScreen.Collapsed ? child.props.children : prev,
        []
      )
    const expandedChildren = React.Children
      .toArray(children)
      .reduce(
        (prev, child) => child.type === MorphingScreen.Expanded ? child.props.children : prev,
        []
      )
    return (
      <Animated.View
        ...
        <Animated.View
          style={[
            ...
            {opacity: this.state.animation.interpolate({inputRange: [0, 1], outputRange: [1, 0]})},
          ]}>
          {collapsedChildren}
        </Animated.View>
