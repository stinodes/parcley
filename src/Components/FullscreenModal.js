...
class FullscreenModal extends React.Component<Props, State> {
  ...
  static getDerivedStateFromProps(props: Props, state: State) {
    if (state.isShown && !props.visible) return { isShown: false };
    return null;
  }
  ...
  get statusBarStyle(): ?'light-content' | 'dark-content' {
    ...
    if (!statusBarStyle) return undefined;
    if (isShown && invertStatusBarStyle) {
      if (statusBarStyle === 'dark-content') return 'light-content';
      return 'dark-content';
    }
  ...
  render() {
    ...
    const { isShown } = this.state;
    return (
      <Modal onAnimate={this.onModalAnimate} {...props}>
        <GSafeArea
          color={color}
