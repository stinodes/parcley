...
import {
  backgroundColor, Button, raised,
  Screen, SystemView as View, Base,
  space, text, flex,
} from 'nativesystem'
...
class NewMatch extends React.Component<ReduxProps<Props, MappedProps>, State> {
  state = {
    joinHeight: null,
  ...
  animate = (toValue: number) => {
    Animated.spring(
      this.state.joinAnimation,
      {toValue}
    ).start()
  }
  
  onJoinContainerLayout = ({nativeEvent: {layout: {height}}}: {nativeEvent: {layout: {height: number}}}) => {
      ...
      <Screen
        ...
        <View f={1} pt={80}>
          <ScrollView
            ...
            <View alignItems="stretch">
              <Base background={Base.Ripple('frenchSky', true)} onPress={Base.delayHandler(this.showJoin)}>
                <View py={2} px={3}>
                  <Text bold modifier="large">Join a match</Text>
                </View>
              </Base>
            </View>
            <CreateMatchForm/>
        ...
        <AbsoluteAnimate
          ...
          <RaisedView raised={19} pt={3} color="white">
            <JoinMatchForm close={this.hideJoin}/>
          </RaisedView>
        </AbsoluteAnimate>
        <Header/>
      </Screen>
...
const ConnectedNewMatch = connect(mapStateToProps)(NewMatch)
export {ConnectedNewMatch as NewMatch}
