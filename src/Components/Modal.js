...
const ModalAnimator = g(Animated.View)(
  absolute,
)
...
type Props = {
  ...
  createAnimation?: ({animation: Animated.Value, toValue: number}) => CompositeAnimation,
  children: React.Node,
}
...
class Modal extends React.Component<Props> {
  ...
  animate = (toValue: 0|1) => {
    ...
    if (createAnimation) {
      return createAnimation({animation: this.animation, toValue})
        .start(this.onEnd)
    }
    
    return Animated.spring(
      this.animation,
      {toValue, useNativeDriver: useNativeDriver}
    ).start(this.onEnd)
  }
